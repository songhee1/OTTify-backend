...
public class ProgramDetailsShowServiceImpl implements ProgramDetailsShowService{
    ...
    public ProgramResponseDto showDetails(Long programId){
        ...
        OAProgramDetailsDto OAProgramDetailsDto =getProgramDetails(program.getTmDbProgramId(),program.getType());
        ProgramDetailResponse programDetailResponse=createProgramDetailResponse(OAProgramDetailsDto);


        //API 요청 두번째: 사람 상세 정보
        OAProgramCreditsDto oaProgramCreditsDto=getCreditsDto(program.getTmDbProgramId(),program.getType());
        changeActorAndDirectorToKorea(oaProgramCreditsDto);

        //API 요청 세번째: Provider 상세 정보
        OAProgramProviderDto oaProgramProviderDto =getProviderDto(program.getTmDbProgramId(),program.getType());
        ...
        // 사용자에게 보내줄 DTO
        ProgramResponseDto programResponseDto=new ProgramResponseDto(programDetailResponse,oaProgramCreditsDto,kr);
    ...
    private OAProgramDetailsDto getProgramDetails(Long tmDbId, ProgramType programType){
        if(programType==ProgramType.Movie){
            ...
            OATvDetailsDto testTvDetailsDto=webClient.get()
                    .uri("/tv/"+tmDbId+"?language=ko")
                    .retrieve()
                    .bodyToMono(OATvDetailsDto.class)
                    .block();

            return testTvDetailsDto;
        }
    }

    //TV와 영화의 API 반환값 차이 제거 위해 새로 DTO 반환값 설계
    private ProgramDetailResponse createProgramDetailResponse(OAProgramDetailsDto oaProgramDetailsDto){
        ...
        oaProgramDetailsDto.getTmDbGenreInfos().stream().forEach(g->{
            if(changeGenreIds.contains(g.getId())){
                g.changeName(genreRepository.findByTmDbGenreId(g.getId()).orElseThrow(()->new NotFoundException(ErrorCode.PROGRAM_GENRE_NOT_FOUND)).getName());
            }
        });
        ...
        String originalCountryName = (oaProgramDetailsDto.getProductionCountries()==null || oaProgramDetailsDto.getProductionCountries().isEmpty())
    ...
    private OAProgramCreditsDto getCreditsDto(Long tmDbProgramId, ProgramType programType){
        if(programType==ProgramType.Movie){
            OAProgramCreditsDto programCreditsDto=webClient.get()
                    .uri("/movie/"+tmDbProgramId+"/credits")
                    .retrieve()
                    .bodyToMono(OAProgramCreditsDto.class)
                    .block();

            return programCreditsDto;
        }
        else{
            OAProgramCreditsDto programCreditsDto=webClient.get()
                    .uri("/tv/"+tmDbProgramId+"/credits")
                    .retrieve()
                    .bodyToMono(OAProgramCreditsDto.class)
                    .block();
    ...
    private OAProgramProviderDto getProviderDto(Long tmDbProgramId, ProgramType programType){
        if(programType==ProgramType.Movie){
            OAProgramProviderDto oaProgramProviderDto =webClient.get()
                    .uri("/movie/"+tmDbProgramId+"/watch/providers")
                    .retrieve()
                    .bodyToMono(OAProgramProviderDto.class)
                    .block();


            return oaProgramProviderDto;
        }
        else{
            OAProgramProviderDto oaProgramProviderDto =webClient.get()
                    .uri("/tv/"+tmDbProgramId+"/watch/providers")
                    .retrieve()
                    .bodyToMono(OAProgramProviderDto.class)
                    .block();
