...
public class ReviewTagServiceImpl implements ReviewTagService {
    ...
    public ReviewTagListResponseDto showReviewTagList() {
        List<ReviewTagInfo> reviewTagInfoList = reviewTagRepository.findAll().stream()
            .map(ReviewTagInfo::new).collect(Collectors.toList());
    ...
    public void basicReviewTagSave() {
        ...
        reviewTags.stream().forEach(reviewTag -> {
            if (!reviewTagRepository.existsByName(reviewTag.getName())) {
                reviewTagRepository.save(reviewTag);
            }
        });
