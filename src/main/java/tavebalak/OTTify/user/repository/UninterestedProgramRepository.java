...
public interface UninterestedProgramRepository extends JpaRepository<UninterestedProgram, Long> {
    ...
    @Query("select new tavebalak.OTTify.user.dto.UninterestedProgramDTO(up.program.id, up.program.posterPath) from UninterestedProgram up where up.user.id =:userId")
